...
simd::float2 UITransform::calculateAnchorPosition(float screenWidth, float screenHeight) const
{
    ...
    if (anchor.target == AnchorTarget::Screen) {
        ...
        simd::float2 anchorPointInParent = getAnchorPointOffset(anchor.point, refWidth, refHeight);
        simd::float2 anchorPointInElement = getAnchorPointOffset(anchor.point, size.x, size.y);
        return anchorBase + anchorPointInParent - anchorPointInElement + simd::float2{anchor.offsetX, anchor.offsetY};
    } else if (anchor.target == AnchorTarget::Parent && parent) {
        if (parent->hasAnchor && parent->anchor.target == AnchorTarget::Screen) {
            simd::float2 parentScreenAnchor = getAnchorPointOffset(parent->anchor.point, screenWidth, screenHeight);
            parentScreenAnchor += simd::float2{parent->anchor.offsetX, parent->anchor.offsetY};
            simd::float2 anchorPointInElement = getAnchorPointOffset(anchor.point, size.x, size.y);
            return parentScreenAnchor - anchorPointInElement + position + simd::float2{anchor.offsetX, anchor.offsetY};
        } else {
            simd::float2 parentPos = parent->getAbsolutePosition();
            simd::float2 parentSize = parent->getSize();
            simd::float2 parentAnchorPoint = getAnchorPointOffset(anchor.point, parentSize.x, parentSize.y);
            simd::float2 anchorPointInElement = getAnchorPointOffset(anchor.point, size.x, size.y);
            return parentPos + parentAnchorPoint - anchorPointInElement + position + simd::float2{anchor.offsetX, anchor.offsetY};
        }
    } else {
        refWidth = screenWidth;
        refHeight = screenHeight;
