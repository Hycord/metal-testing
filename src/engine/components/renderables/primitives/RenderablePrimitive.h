...
public:
    ...
    void setDepthBias(float bias, float slopeScale = 0.0f) {
        ...
        if (auto renderable = registeredRenderable.lock())
            renderable->setDepthBias(bias, slopeScale);
    }
...
protected:
    ...
    std::shared_ptr<Renderable> makeRenderable(MTL::Device *device,
        ...
        Shader *shader = new Shader(device, "General", "vertexGeneral", "fragmentGeneral", mesh.vertexDescriptor, true);
        ...
        renderable->setPrimitiveType(type);
        renderable->setScreenSpace(screenSpace);
        renderable->setDepthBias(depthBias, depthBiasSlopeScale);
