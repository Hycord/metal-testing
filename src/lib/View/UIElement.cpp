...
UIElement::UIElement(MTL::Device* device, float screenWidth, float screenHeight)
    : device(device), screenWidth(screenWidth), screenHeight(screenHeight), quadShader(nullptr), quadMaterial(nullptr), quadRenderable(nullptr)
{
}
...
void UIElement::setScreenSize(float sw, float sh)
{
...
{
    // default: no-op. Derived classes can rebuild caches here.
}
