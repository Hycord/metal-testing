#include "components/renderables/primitives/2d/CirclePrimitive.h"
...
void CirclePrimitive::rebuild()
{
    ...
    vertices[0] = {{cx, cy, 0.0f}, {color.x, color.y, color.z}, {0.0f, 0.0f}};
    ...
    for (int i = 0; i <= segments; ++i)
    {
        ...
        float x = cx + p.x;
        float y = cy + p.y;
        vertices[i + 1] = {{x, y, 0.0f}, {color.x, color.y, color.z}, {0.0f, 0.0f}};
    }
